...
request_records = {}
# 创建线程锁
request_records_lock = threading.Lock()
...
def upload_file():
    ...
    try:
        ...
        if result:
            ...
            # 更新请求记录状态
            with request_records_lock:
                request_records[request_id]['status'] = 'completed'
            return result
        else:
        ...
        # 更新请求记录状态
        with request_records_lock:
            if request_id in request_records:
                request_records[request_id]['status'] = 'error'
        return str(e), 500
